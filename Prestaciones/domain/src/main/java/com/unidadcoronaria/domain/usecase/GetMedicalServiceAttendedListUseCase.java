 ...
 * @since 0.0.1
 */
public class GetMedicalServiceAttendedListUseCase extends UseCase<List<MedicalService>> {

    private final MedicalServiceTransformer transformer = new MedicalServiceTransformer();

    @Override
    public void execute(Context aContext) {
        ApiClient.getInstance().getMedicalServiceAttendedList(new SuccessFailureCallBack<List<MedicalServiceEntity>>() {
            @Override
            ...
            public void onFailure(String message) {
                GetMedicalServiceAttendedListUseCase.super.onFailure(message);
            }
        });
    }
